...
class NativeClarityBinProvider {
    constructor(allocations, dbFilePath, clarityBinPath) {
        this.closeActions = [];
     ...
     */
    static async create(allocations, dbFilePath, clarityBinPath) {
    ...
    async eval(contractName, evalStatement, includeDebugOutput, atChaintip = true) {
        const result = await this.runCommand([`eval${atChaintip ? "_at_chaintip" : ""}`, contractName, this.dbFilePath], {
            stdin: evalStatement,
        });
        if (result.exitCode !== 0) {
            throw new ExecutionError(
            // tslint:disable-next-line: max-line-length
            `Eval expression on contract failed with bad exit code ${result.exitCode}: ${result.stderr}`, result.exitCode, result.stdout, result.stderr);
        }
        // Check and trim success prefix line.
        const successPrefix = result.stdout.match(/(Program executed successfully! Output: (\r\n|\r|\n))/);
        if (!successPrefix || successPrefix.length < 1) {
